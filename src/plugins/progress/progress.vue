...
<script>
  ...
  export default {
	...
	setup(props){
		const {
			percentage,color,strokeWidth,width,type,status,format
		} = toRefs(props)
		...
		const getCurrentColor = (percentage) => {
			if (typeof color.value === 'function') {
			  return color.value(percentage.value);
		...
		const getColorArray = () => {
			...
			const span = 100 / col.length;
			return col.map((seriesColor, index) => {
		...
		const stroke = useStroke(color,percentage,status,getCurrentColor)
  ...
  const useRelativeStrokeWidth = (strokeWidth,width) => {
	 const elativeStrokeWidth = computed(() => {
		return  (strokeWidth.value / width.value * 100).toFixed(1);
	 })
	  
	  return elativeStrokeWidth 
  }
  ...
  const useStroke = (color,percentage,status,getCurrentColor) => {
